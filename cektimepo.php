...
$sql01     = "
		...
		from (
			...
			from new_pro_inventory_vendor_po
...
if (count($res01) > 0) {
    ...
    $pesn = "Sistem SYOP meminta persetujuan untuk PO Supplier, dikarenakan dalam jangka waktu 20 menit, CFO belum menyetujui PO Supplier ini";

    foreach ($res01 as $data) {
        $idmaster     = $data['id_master'];
        $sql02     = "
				update new_pro_inventory_vendor_po set cfo_summary = 'Approved By System Automatically', cfo_result = 1, cfo_pic = 'Syop System', cfo_tanggal = NOW(), 
				disposisi_po = 2
