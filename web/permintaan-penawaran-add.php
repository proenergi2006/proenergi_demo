...
inner join acl_role c on b.id_role = c.id_role where id_pmnt = '".$idr."'";
		...
		$rsm = array();
		$action 	= "add";
		$titleAct 	= "Tambah Permintaan Penawaran";
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="box box-primary">
                            ...
                            <div class="box-body">
                                <form action="<?php echo ACTION_CLIENT.'/permintaan-penawaran.php'; ?>" id="gform" name="gform" method="post"  enctype="multipart/form-data">
                                <div class="form-group row">
                                    <div class="col-sm-6">
										<label>Judul *</label>
                                        <input type="text" name="judul_pmnt" id="judul_pmnt" class="form-control validate[required]" value="<?php echo isset($rsm['judul_pmnt']) ? strip_tags($rsm['judul_pmnt']) : '';?>" autocomplete="off" />
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-10 col-md-8">
                                        <label>Permintaan</label>
                                        <textarea name="pesan_pmnt" id="pesan_pmnt" class="form-control wysiwyg"><?php echo $rsm['pesan_pmnt'] ?? null;?></textarea>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-12">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="pad bg-gray">
                                            ...
                                            <a class="btn btn-default jarak-kanan" href="<?php echo BASE_URL_CLIENT."/permintaan-penawaran.php";?>">
	...
	$(document).ready(function(){
		$("form#gform").validationEngine('attach',{
			onValidationComplete: function(form, status){
				...
				if(status == true){
					CKEDITOR.instances.pesan_pmnt.updateElement();
					if($("#pesan_pmnt").val() == ""){
						$('#preview_modal').find('#preview_alert').html('<p class="text-center">Permintaan belum diisi</p>');
						$('#preview_modal').modal();
					}else {
						$('#preview_modal').find('#preview_alert').html('<p class="text-center">Permintaan penawaran tidak dapat diubah, Apakah anda yakin ?</p>');
