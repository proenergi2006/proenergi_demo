<?php
	...
	if($q5){
		$q5Txt = $con->getOne("select nama_kab from pro_master_kabupaten where id_master = '".$q5."'");
		$writer->writeSheetHeaderExt($sheet, array("Wilayah Kirim : ".$q5Txt=>"string"));
		$writer->newMergeCell($sheet, "A".$start, "O".$start);
		$patok++;
		$start++;
	}
	...
	if(count($res) > 0){
		...
		foreach($res as $data){
			...
			$writer->writeSheetRow($sheet, array(
				...
				'=I'.$last.'-J'.$last.'', $rating, $data['nama_area'], $data['nama_cabang'], $alatUk, 
			));
		}
		$writer->writeSheetRow($sheet, array(
			"", "", "", "", "", "", "", "TOTAL", 
