<?php
	...
	$link = BASE_URL_CLIENT.'/report/l-volume-angkut-exp.php?'.paramEncrypt('q1='.$q1.'&q2='.$q2.'&q3='.$q3.'&q4='.$q4.'&q5='.$q5);
		...
		foreach($result as $data){
        	...
        	$content .= '
				<tr>
					...
					<td class="text-left">'.$tempal.'</td>
					<td class="text-right">'.number_format($data['volume']).'</td>
				</tr>';
		}
		$content .= '
			<tr>
				<td class="text-center bg-gray" colspan="2"><b>TOTAL</b></td>
				<td class="text-right bg-gray"><b>'.number_format(($tot_vol)).'</b></td>
			</tr>';
		$content .= '<tr class="hide"><td colspan="3"><input type="hidden" id="uriExp" value="'.$link.'" /></td></tr>';
	} 
