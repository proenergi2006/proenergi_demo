...
foreach ($data_ as $i => $row) {
    $ongkos_po_real = $row->ongkos_po_real;
    ...
    $content[] = array(
        ...
        $row->nama_cabang,
        $row->nama_terminal . " " .  $row->tanki_terminal,
        $row->no_spj,
        $row->nomor_plat,
        $row->nama_sopir,
        $row->nama_transportir,
        isset($arrMobil[$row->pr_mobil]) ? $arrMobil[$row->pr_mobil] : '',
        $row->nama_customer,
        $row->nama_kab,
        $row->volume_po,
        trim($row->nomor_poc),
        $row->nomor_po,
        $row->ongkos_po_real,
        ($row->ongkos_po_real * $row->volume_po),
        $tanggal_jam_eta,
        !empty($row->drop_point_start) ? date('d-m-Y H:i', strtotime($row->drop_point_start)) : '',
        $tanggal_jam_loaded,
        !empty($row->tanggal_delivered) ? date('d-m-Y H:i', strtotime($row->tanggal_delivered)) : '',
        '',
        !empty($row->tgl_realisasi) ? date('d-m-Y H:i', strtotime($row->tgl_realisasi)) : '',
        timeManHours($leadtm),
