<?php
	...
	$sql = "
		...
		from (
			...
			join acl_user g on f.id_marketing = g.id_user 
			where a.is_delivered = 1 ".$where." 
		) a group by bulan_delivered, id_area, id_marketing, fullname, nama_area";
	if(isset($_POST['chart']) && $_POST['chart']=='1'){
		$sql = "
		select fullname , sum(jum_vol) as volume 
		from (
			select extract(year_month from a.tanggal_delivered) as bulan_delivered, b.volume_po as jum_vol, f.id_marketing, g.fullname, d.id_area, e.nama_area 
			from pro_po_ds_detail a
			join pro_po_detail b on a.id_pod = b.id_pod 
			...
			join acl_user g on f.id_marketing = g.id_user 
			where a.is_delivered = 1 ".$where." 
		) a group by id_marketing, fullname, nama_area
		";
		$result 	= $con->getResult($sql);
		$chart	= array();
		foreach($result as $data){
			$chart['data'][] = (int)$data['volume'];
			$chart['name'][] = $data['fullname'];
		}
	...
	$link = BASE_URL_CLIENT.'/report/m-sales-performance-exp.php?'.paramEncrypt('q1='.$q1.'&q2='.$q2.'&q3='.$q3.'&q4='.$q4);

	$content = "";
	$count = 0;
	if($tot_record == 0){
		$content .= '<tr><td colspan="4" style="text-align:center"><input type="hidden" id="uriExp" value="'.$link.'" />Data tidak ditemukan </td></tr>';
	} else{
		$count 		= $position;
		...
		$content .= '
			<tr>
				<td class="text-center bg-gray" colspan="3"><input type="hidden" id="uriExp" value="'.$link.'" /><b>TOTAL</b></td>
