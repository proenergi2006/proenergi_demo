<?php
    ...
    foreach($data_ as $i=>$row) {
        $content[] = array(
            ($i+1),
            $row->nama_customer,
            number_format($row->potensi_volume),
            $row->potensi_waktu,
            $row->tersuplai_jumlah_pengiriman,
            $row->tersuplai_waktu,
            number_format($row->tersuplai_volume),
            $row->sisa_potensi,
            $row->kompetitor,
    ...
    $filename = "Data-Database-Fuel-".date('dmYHis').'.xlsx';
    ...
    $writer->writeSheetHeader($sheet, array('Data Database Fuel'=>'string'));
    ...
    $header = array(
        ...
        'Potensi Volume' => 'string',
        'Potensi Waktu' => 'string',
        'Tersuplai Jumlah Pengiriman' => 'string',
        'Tersuplai Waktu' => 'string',
        'Tersuplai Volume' => 'string',
        'Sisa Potensi' => 'string',
        'Kompetitor' => 'string',
        'Harga Kompetitor' => 'string',
        'PIC' => 'string',
        'TOP' => 'string',
        'Kontak Email' => 'string',
    ...
    if (count($data_) > 0) {
        ...
        $writer->newMergeCell($sheet, 'A4', 'P4');
    }
