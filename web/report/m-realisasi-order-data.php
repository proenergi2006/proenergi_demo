<?php
	...
	$q1	= isset($_POST["q1"])?htmlspecialchars($_POST["q1"], ENT_QUOTES):null;
	$q2	= isset($_POST["q2"])?htmlspecialchars($_POST["q2"], ENT_QUOTES):null;
	$q3	= isset($_POST["q3"])?htmlspecialchars($_POST["q3"], ENT_QUOTES):null;
	$q4	= isset($_POST["q4"])?htmlspecialchars($_POST["q4"], ENT_QUOTES):null;
	$q5	= isset($_POST["q5"])?htmlspecialchars($_POST["q5"], ENT_QUOTES):null;
	$q6	= isset($_POST["q6"])?htmlspecialchars($_POST["q6"], ENT_QUOTES):null;
	$q7	= isset($_POST["q7"])?htmlspecialchars($_POST["q7"], ENT_QUOTES):null;
	...
	$realisasi_volume = false;
	$s1 = $realisasi_volume?', realisasi_volume':null;
	$s2 = $realisasi_volume?', a.realisasi_volume':null;
	$s3 = $realisasi_volume?', 0 as realisasi_volume':null;
	$g1 = $realisasi_volume?', realisasi_volume':null;
	$td1 = $realisasi_volume?(paramDecrypt($_SESSION["sinori".SESSIONID]["id_role"]) == 6?'<td class="text-right">'.number_format($data['realisasi_volume']).'</td>':''):null;
	...
	$sql = "
		...
		id_area, nama_area, harga_asli, harga_minyak ".$s1." 
		from (
			...
			k.nama_kab, f.id_marketing, g.fullname, d.id_area, e.nama_area, d.harga_asli, l.harga_minyak ".$s2."
			...
			k.nama_kab, f.id_marketing, g.fullname, d.id_area, e.nama_area, d.harga_asli, l.harga_minyak ".$s3."  
			...
			left join pro_master_harga_pertamina l on d.masa_awal = l.periode_awal and d.masa_akhir = l.periode_akhir and d.id_area = l.id_area and d.produk_tawar = l.id_produk 
			where a.is_delivered = 1 ".$where." 
		) a group by tanggal_delivered, id_customer, nama_customer, id_wilayah, nama_cabang, nama_kab, kab_survey, id_marketing, fullname, nama_area, id_area, harga_asli, harga_minyak ".$g1;
	if($q4 && $q5){
		$arrOp = array(1=>"=", ">=", "<=");
	...
	$link = BASE_URL_CLIENT.'/report/m-realisasi-order-exp.php?'.paramEncrypt('q1='.$q1.'&q2='.$q2.'&q3='.$q3.'&q4='.$q4.'&q5='.$q5.'&q6='.$q6.'&q7='.$q7.'&q8='.$q8.'&q9='.$q9);
	...
	if($tot_record == 0){
		...
		foreach($result as $data){
			...
			$arHj = isset($data['detail_rincian'])?json_decode($data['detail_rincian'], true):null;
			$total = 0;
        	...
        	$content .= '
				<tr>
					...
					<td class="text-right">'.($disc?number_format($disc):0).'</td>
				</tr>';
		}
		$nom1 = ($nom1)?$nom1:1;
		$content .= '
			<tr>
				...
				<td class="text-center bg-gray"><input type="hidden" id="uriExp" value="'.$link.'" /><b>AVERAGE</b></td>
