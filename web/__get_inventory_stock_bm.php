...
$sql = "select
        ...
        t.lokasi_terminal,
        p.merk_dagang,
        (SUM(i.in_inven) - SUM(i.out_inven) + SUM(i.adj_inven) - SUM(i.out_inven_virtual)) AS stock_terakhir
        FROM
        pro_inventory_depot i
        JOIN
        pro_master_terminal t ON i.id_terminal = t.id_master
        JOIN
        pro_master_produk p ON i.id_produk = p.id_master
        WHERE
        i.id_terminal IN (2, 3, 23, 49)
        AND i.tanggal_inven = (
        SELECT MAX(tanggal_inven)
        FROM pro_inventory_depot
        WHERE id_terminal = i.id_terminal
        AND id_produk = i.id_produk 
...
foreach ($rows as $row) {
    ...
    $chartData = [
        ...
        'oilLevel' => (float)$row['stock_terakhir'], // Ganti dengan kolom yang sesuai
...
$query = "select a.*, 
            ...
            where poc_approved = 1 and b.id_wilayah = '" . $seswil . "'";
