...
<body class="skin-blue fixed">
				...
				<div class="tab-content">
							...
							<div class="form-group row">
								...
								<div class="col-sm-3 col-sm-top">
									<select id="q2" name="q2" class="form-control">
										<option></option>
										<option value="1">Tanggal PO Transportir</option>
								...
								<div class="col-sm-3 col-sm-top">
									<div class="input-group">
										...
										<input type="text" name="q3" id="q3" class="form-control input-sm datepicker" />
									</div>
								</div>
								<div class="col-sm-3 col-sm-top">
									<div class="input-group">
										<span class="input-group-addon">S/D</span>
										<input type="text" name="q4" id="q4" class="form-control input-sm datepicker" />
									</div>
								</div>
							</div>
							<div class="form-group row">
								<div class="col-sm-3">
									<select id="q5" name="q5" class="form-control select2">
										<option></option>
										<option value="1">Belum Loading</option>
										<option value="2">Loaded</option>
										<option value="3">Delivered</option>
										<option value="4">Cancel</option>
										<option value="5">Belum Loading & Cancel</option>
									</select>
								</div>
										...
										<table class="table table-bordered" id="data-truck-table">
											<thead>
												<tr>
													...
													<th class="text-center" width="13%">PO Customer</th>
					...
					<div role="tabpanel" class="tab-pane" id="data-kapal">
						<form name="searchForm2" id="searchForm2" role="form" class="form-horizontal">
							...
							<div class="form-group row">
								<div class="col-sm-3">
									<select id="q4k" name="q4k" class="form-control select2">
										<option></option>
									...
									<tbody>
										...
										<tr>
											...
											<td style="padding:10px 5px;" class="teradepo">
												...
												<input type="text" name="tera_depo" id="tera_depo" class="form-control teradepo" />
											</td>
										...
										<tr class="losses-field hide">
											<td colspan="5" style="padding:10px 5px;">
												<p style="font-weight:bold; margin-bottom:5px;">Attachment Bukti Losses</p>
												<input type="file" name="losses_file" id="losses_file" class="form-control" />
											</td>
										</tr>
										<tr class="losses-field hide">
											<td colspan="5" style="padding:10px 5px;">
												<p style="font-weight:bold; margin-bottom:5px;">Catatan Losses</p>
												<textarea name="catatan_losses" id="catatan_losses" class="form-control" rows="3"></textarea>
											</td>
										</tr>
										<tr>
	...
	<div class="modal fade" id="request_load_modal" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header bg-blue">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				...
				<div class="modal-body">
					<div id="infoStatLP"></div>
					<div id="errStatRP"></div>
					<table class="table no-border">
						<tbody>
							<div class="row">
								<div class="col-sm-6">
									<p id="lb5" style="font-weight:bold; margin-bottom:5px;"></p>
									<select id="dt5_load" name="dt5_load" class="form-control select2">
										<option></option>
							...
							<div class=" row">
								<div class="col-sm-12">
									<p id="ctt" style="font-weight:bold; margin-bottom:5px;"></p>
									<textarea name="dt6_load" id="dt6_load" class="input-sm form-control" rows="8" autocomplete="off"></textarea>
								</div>
							</div>
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="pad bg-gray">
								<input type="hidden" name="pic_mkt" id="pic_mkt" value="" style="font-weight: bold;" />
								<input type="hidden" name="is_loaded" id="is_loaded" value="" style="font-weight: bold;" readonly />
								<input type="hidden" name="loco" id="loco" value="" style="font-weight: bold;" readonly />
								<input type="hidden" name="masa_akhir" id="masa_akhir" value="" style="font-weight: bold;" readonly />
								<input type="hidden" name="paramRQ" id="paramRQ" value="" />
								<button type="button" class="btn btn-default jarak-kanan" data-dismiss="modal">Batal</button>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$("#data-truck-table").ajaxGrid({
				url: "./datatable/pengiriman-list-logistik-truck.php",
				data: {
					q1: $("#q1").val(),
					q2: $("#q2").val(),
					q3: $("#q3").val(),
			...
			$("[data-toggle='tab']").click(function() {
				...
				var urlnya = (idnya == "#data-truck") ? "./datatable/pengiriman-list-logistik-truck.php" : "./datatable/pengiriman-list-logistik-kapal.php";
			...
			$('#data-truck-table tbody, #data-kapal-table tbody').on('click', '.editStsT', function(e) {
				...
				$.post("./datatable/get_info_loading.php", {
					...
					// Ambil nilai volume dari elemen hidden
					let vol = parseFloat($("#status_kirim_modal").find("#volume_detail_hidden").val()) || 0;

					// Masukkan ke input #volume (dimanapun input itu berada)
					$("#volume").val(vol);
			...
			$("#status_history_modal").on("shown.bs.modal", function() {
				...
				$.ajax({
					...
					success: function(data) {
						if (data) {
							if (jns == 1) $("#data-truck-table").ajaxGrid("draw");
			...
			$("#status_load_modal").on("hidden.bs.modal", function() {
				...
				// Ambil tanggal dari input dt1_load dan etl_val
				var dt1_load = $("#dt1_load").val();
				var etl_val = $("#etl_val").val();

				// Parsing tanggal input menjadi objek Date
				var inputDate = new Date(dt1_load.split("/").reverse().join("-")); // format input "dd/mm/yyyy" dibalik jadi "yyyy-mm-dd"
			...
			$("#status_kirim_modal").on("shown.bs.modal", function() {
				...
				if ($("#stat_kirim").val() == "" || $("#dt1").val() == "" || $("#dt2").val() == "" || $("#dt3").val() == "") {
					...
					$.ajax({
						...
						success: function(data) {
							$("#loading_modal").modal("hide");
							if (tipe == 1) $("#data-truck-table").ajaxGrid("draw");
							else $("#data-kapal-table").ajaxGrid("draw");
							if (data.error) {
								$("#error_modal").find("#error-preview").html(data.error);
								$("#error_modal").modal({
				...
				if (confirm("Apakah anda yakin ?")) {
					...
					const volume = parseFloat($("#volume").val().replace(/,/g, "")) || 0;
					const real = parseFloat($("#real_kirim").val().replace(/,/g, "")) || 0;

					// jika real < volume, wajib isi losses
					if (real < volume) {
						$(".losses-field").removeClass("hide");
						if (!$("#catatan_losses").val().trim()) {
							Swal.fire({
								icon: "error",
								title: "Terindikasi Losses",
								text: "Realisasi lebih kecil dari volume pengiriman. Harap isi catatan losses!",
							});
							return;
						}
					}
					...
					// ambil tipe
					var tipe = $("#tipeLP").val(); // <-- titik-koma, bukan koma

					// siapkan FormData
					const formData = new FormData();
					formData.append("file", "logistik");
					formData.append("aksi", "realisasi");
					formData.append("dt1", $("#dt1").val());
					formData.append("dt2", $("#dt2").val());
					formData.append("dt3", $("#dt3").val());
					formData.append("dt4", $("#real_kirim").val());
					formData.append("dt5", $("#staj_kirim").val());
					formData.append("param", $("#idLP").val());
					formData.append("tipe", tipe);
					formData.append("tera_depo", $("#tera_depo").val());
					formData.append("tera_site", $("#tera_site").val());

					if (real < volume) {
						formData.append("catatan_losses", $("#catatan_losses").val());
						formData.append("losses_file", $("#losses_file")[0].files[0]);
					}

					// show loading dan hide status modal
					$("#loading_modal").modal({
						keyboard: false,
						backdrop: "static"
					});
			...
			$('#data-truck-table tbody, #data-kapal-table tbody').on('click', '.editStsLoading', function(e) {
				var param = $(this).data("param");
			...
			$('#data-truck-table tbody, #data-kapal-table tbody').on('click', '.editStsRequest', function(e) {
				...
				$.post("./datatable/get_info_loading.php", {
					...
					$("#request_load_modal").find("#pic_mkt").val(mkt);
