...
$sqlPeriods = "
  SELECT
    ...
    (
      ...
      WHERE p.id_customer = h.id_customer
        AND p.id_marketing = h.id_marketing
    ...
    (
      ...
      WHERE po.id_customer = h.id_customer
        AND po.id_marketing = h.id_marketing
  ...
  WHERE h.id_customer = " . (int)$idr . "
  ORDER BY h.effective_from DESC
";
$periods = $con->getResult($sqlPeriods);

$sqlAll = "
    /* A) PO yang jatuh pada periode & marketing tsb (dengan penawaran jika ada) */
    SELECT
    h.id AS hist_id,
    h.id_customer,
    h.id_marketing,
    ...
    COALESCE((SELECT COALESCE(volume_close, 0)
                ...
                WHERE id_poc = po.id_poc AND st_Aktif = 'Y'),0) as volume_close_po,

    COALESCE(planagg.realisasi, 0) AS realisasi,
    p.id_penawaran,
    p.nomor_surat        AS nomor_penawaran,

    0 AS is_penawaran_only   -- flag

    FROM pro_customer_marketing_history h
    LEFT JOIN pro_po_customer po
        ON po.id_customer  = h.id_customer
        AND po.id_marketing = h.id_marketing
        AND po.tanggal_poc >= h.effective_from
        AND (h.effective_to IS NULL OR po.tanggal_poc < h.effective_to)
    LEFT JOIN (
        SELECT
            pp.id_poc,
        ...
        ON planagg.id_poc = po.id_poc
    LEFT JOIN pro_penawaran p
        ON p.id_penawaran  = po.id_penawaran
        AND p.id_customer   = po.id_customer
        AND p.id_marketing  = po.id_marketing
    WHERE h.id_customer = " . (int)$idr . "
     AND po.id_poc IS NOT NULL

    UNION ALL

    /* B) Penawaran dalam periode & marketing tsb yang BELUM punya PO */
    SELECT
    h.id AS hist_id,
    h.id_customer,
    h.id_marketing,
    ...
    AND po.id_marketing = p.id_marketing
    AND po.tanggal_poc >= h.effective_from
    AND (h.effective_to IS NULL OR po.tanggal_poc < h.effective_to)
    WHERE h.id_customer = " . (int)$idr . "
    AND po.id_poc IS NULL

    ORDER BY
    effective_from DESC,
    COALESCE(tanggal_poc) DESC,
    id_poc DESC";
$rowsAll = $con->getResult($sqlAll);

// 3) Group ke hist_id => list PO (tetap)
$listPoPerPeriode = [];
foreach ($rowsAll as $r) {
    $hid = (int)$r['hist_id'];
    if (!isset($listPoPerPeriode[$hid])) $listPoPerPeriode[$hid] = [];
    // simpan semua baris (baik yang ada PO maupun penawaran-only)
    $listPoPerPeriode[$hid][] = [
        'id_poc'           => $r['id_poc'] !== null ? (int)$r['id_poc'] : null,
        'nomor_poc'        => $r['nomor_poc'],            // bisa "Belum PO"
        'tanggal_poc'      => $r['tanggal_poc'],
        'volume_poc'       => isset($r['volume_poc']) ? (float)$r['volume_poc'] : 0,
        'volume_close_po'  => isset($r['volume_close_po']) ? (float)$r['volume_close_po'] : 0,
        'realisasi'        => isset($r['realisasi']) ? (float)$r['realisasi'] : 0,
