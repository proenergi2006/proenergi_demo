                    ...
                    <div class="box box-info">
                        <div class="box-header with-border">
                            <div class="row">
                            	<div class="col-sm-6">
                                    <?php if ($sesrole==11) { ?>
                                    <a href="<?php echo BASE_URL_CLIENT.'/database-lubricant-oil-add.php'; ?>" class="btn btn-primary">
                        ...
                        <div class="box-body table-responsive">
                            <table class="table table-bordered table-hover" id="data-database-lubricant-oil-table">
                                <thead>
                                    <tr>
...
<style>
	#data-database-lubricant-oil-table td, #data-database-lubricant-oil-table th {font-size:12px;}
</style>
<script>
$(document).ready(function(){
	$("#data-database-lubricant-oil-table").ajaxGrid({
		url	 : "./datatable/database-lubricant-oil.php",
		data : {q1 : $("#q1").val(), q2 : $("#q2").val(), q3 : $("#q3").val()},
	});
	$('#btnSearch').on('click', function(){
		var param = {q1 : $("#q1").val(), q2 : $("#q2").val(), q3 : $("#q3").val()};
		$("#data-database-lubricant-oil-table").ajaxGrid("draw", {data : param}); 
		return false;
	});
	$('#tableGridLength').on('change', function(){
		$("#data-database-lubricant-oil-table").ajaxGrid("pageLen", $(this).val());
	});
	$('#data-database-lubricant-oil-table').on('click', '[data-action="deleteGrid"]', function(e){
		e.preventDefault();
		if(confirm("Apakah anda yakin ?")){
			...
			var handler	= function(data){
				if(data.error == ""){
					$(".alert").slideUp();
					$("#data-database-lubricant-oil-table").ajaxGrid("draw");
				} else{
					$(".alert").slideUp();
    ...
    $('#btnExport').on('click', function () {
        let url = '<?php echo BASE_URL_CLIENT ?>/report/e-export-database-lubricant-oil-xls.php';
