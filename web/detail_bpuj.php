...
$data 	= "SELECT a.*, b.nomor_do FROM pro_bpuj a JOIN pro_po_ds_detail b ON a.id_dsd=b.id_dsd WHERE a.id_bpuj='" . $idr . "' AND a.disposisi_bpuj != '0' AND a.is_active='1'";
...
// $created_time_bpuj = date("H:i:s", strtotime($row['tanggal_bpuj']));
...
if ($realisasi) {
	...
	if ($realisasi['pengisian_bbm_tambahan2'] != NULL) {
		...
		$pengisian_bbm_realisasi3 = $exp_realisasi3[0];
		$id_terminal_realisasi3 = $exp_realisasi3[1];
		if ($realisasi['dispenser_tambahan2'] != 0) {
			$query_dispenser_realisasi3 = "SELECT * FROM pro_master_terminal WHERE id_master = '" . $id_terminal_realisasi3 . "'";
			$dispenser_realisasi3 = $con->getRecord($query_dispenser_realisasi3);
		}
	} else {
		$pengisian_bbm_realisasi3 = $exp3[0];
		$dispenser_realisasi3 = $dispenser3;
	}
}
...
<body class="skin-blue fixed">
	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
		...
		<aside class="right-side">
			...
			<section class="content">
				<?php if ($row) : ?>
					<input type="hidden" name="id" id="id" readonly="readonly" value="<?php echo paramEncrypt($row['id_bpuj']) ?>">
					<input type="hidden" name="id_realisasi" id="id_realisasi" readonly="readonly" value="<?php echo paramEncrypt($realisasi['id']) ?>">
					<div class="row">
						<div class="col-md-12">
							<table width="100%" border="0">
								...
								<?php if ($realisasi) : ?>
									...
									<?php endif ?>
									<?php if ($realisasi['pengisian_bbm_tambahan2'] != NULL) : ?>
										<tr>
											<td>
												<b>Pengisian BBM Realisasi</b>
											</td>
											...
											<td>
												<?php
												if ($realisasi['dispenser_tambahan2'] != 0) {
													echo $dispenser_realisasi3['nama_terminal'] . " - " . $dispenser_realisasi3['tanki_terminal'];
												} else {
													echo $pengisian_bbm_realisasi3;
												}
												?>
											</td>
											<td>
											...
											<td>
												<?php if (fmod($realisasi['liter_bbm_tambahan2'], 1) !== 0.000) : ?>
													<?= number_format($realisasi['liter_bbm_tambahan2'], 3, ",", ".") ?> Liter
												<?php else : ?>
													<?= number_format($realisasi['liter_bbm_tambahan2']) ?> Liter
												<?php endif ?>
												<input type="hidden" name="liter_bbm3" id="liter_bbm3" value="<?= $realisasi['liter_bbm_tambahan2'] ?>">
											</td>
											<?php if ($realisasi['tgl_pengisian_tambahan2'] != NULL) : ?>
												<td>
													<b>Tgl Pengisian</b>
												</td>
												<td>
												...
												<td>
													<?= tgl_indo($realisasi['tgl_pengisian_tambahan2']) ?>
												</td>
											<?php endif ?>
										</tr>
									<?php endif ?>
						...
						<div class="col-md-6">
							...
							<table width="100%" border="1">
								...
								<tr>
									...
									<?php if ($row['diberikan_oleh'] == NULL) : ?>
										<td class="text-right">
											<input type="text" name="yang_dibayarkan" id="yang_dibayarkan" class="form-control text-right" onkeypress="return onlyNumberKey(event)" placeholder="0">
										</td>
									<?php else : ?>
										<td class="text-right">
							...
							<br>
							<?php if ($row['disposisi_bpuj'] == "1") : ?>
								<div class="row">
									<div class="col-md-12">
										<button type="button" class="btn btn-primary" id="btnSimpan">Simpan</button>
										<button type="button" class="btn btn-success" id="btnKembalikan">Kembalikan ke Logistik</button>
									</div>
								</div>
							<?php endif ?>
						</div>
									...
									<?php else : ?>
										<div class="row">
											<div class="col-md-12">
												<?php if ($sesrol != '25') : ?>
													<center>
														<a type="button" class="margin-sm btn btn-success btn-md" title="Approve Realisasi" id="approve"><i class="fa fa-thumbs-up"></i> Approve</a>
													</center>
												<?php endif ?>
											</div>
										</div>
										...
										<td colspan="2">
											...
											<small>
												<?php if ($realisasi['catatan'] != NULL) : ?>
													<textarea id="catatan" style="border: none; outline: none; resize:none;" readonly><?= $realisasi['catatan'] ?></textarea>
												<?php else : ?>
													<span id="catatan">-</span>
												<?php endif ?>
											</small>
											<br>
											<?php if ($realisasi['disposisi_realisasi'] == 0) : ?>
												<textarea class="form-control hide" id="catatan_edit" name="catatan_edit"><?= $realisasi['catatan'] ?></textarea>
												<br>
												<?php if ($sesrol != '25') : ?>
													<button type="button" id="btn-edit-catatan" class="btn btn-primary btn-sm">Edit</button>
												<?php endif ?>
												<button type="button" id="btn-batal-catatan" class="btn btn-danger btn-sm hide">Batal</button>
											<?php endif ?>
										</td>
									</tr>
									<tr>
	...
	<script>
		...
		$("#yang_dibayarkan").keyup(function() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					if (bayar == "") {
						Swal.fire({
							title: "Oopss",
							text: "Masukan nominal pada kolom yang dibayarkan",
							icon: "warning"
						});
					} else {
						$("#loading_modal").modal({
							...
							if (result.status == false) {
								...
								setTimeout(function() {
									...
									Swal.fire({
										...
										location.href = '<?php echo BASE_URL_CLIENT ?>/list_bpuj.php?'
									});
								}, 2000);
							}
						},
		...
		$('#approve').click(function() {
			...
			var catatan_edit = $("#catatan_edit").val();
			Swal.fire({
		...
		$("#btn-edit-catatan").click(function() {
			$("#catatan_edit").removeClass('hide');
			$("#btn-batal-catatan").removeClass('hide');
			$("#btn-edit-catatan").addClass('hide');
		})
		$("#btn-batal-catatan").click(function() {
			$("#catatan_edit").addClass('hide');
			$("#catatan_edit").val(`<?= $realisasi['catatan'] ?>`);
			$("#btn-edit-catatan").removeClass('hide');
			$("#btn-batal-catatan").addClass('hide');
		})
	</script>
</body>
