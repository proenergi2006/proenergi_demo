...
<div class="modal fade" id="fullscreenModal" tabindex="-1" role="dialog" aria-labelledby="fullscreenModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-fullscreen-custom" role="document">
		<div class="modal-content modal-content-fullscreen">
			<div class="modal-header">
				<h5 class="modal-title" id="fullscreenModalLabel">GPS Tracking, Powered by SinarGPS</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Tutup">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<div style="width: 100%; height: 500px; border: 1px solid #ccc;" id="gps">
					<iframe
						id="gpsIframe"
						src=""
						width="100%"
...
<body class="skin-blue fixed">
						...
						<?php if ($sesrole == '1' || ($sesrole == '14' && $seswil == '1')) : ?>
							<div class="container-fluid">
								<div class="row">
									...
									<div class="col-md-6">
										<div class="form-group form-group-sm">
											<select id="id_cabang" name="id_cabang" class="form-control select2" style="width:100%;">
												<option></option>
												<?php
												$sqlnya01 = "SELECT id_master, nama_cabang, inisial_cabang FROM pro_master_cabang WHERE is_active = 1";
												$resnya01 = $con->getResult($sqlnya01);

												if (count($resnya01) > 0) {
													foreach ($resnya01 as $idx01 => $data01) {
														// Cek apakah id_master sama dengan $seswil
														$selected = ($data01['id_master'] == $seswil) ? 'selected' : '';

														echo '<option value="' . $data01['id_master'] . '" data-inisial-cabang="' . $data01['inisial_cabang'] . '" ' . $selected . '>' . $data01['nama_cabang'] . '</option>';
													}
												}
												?>
											</select>
										...
										<div class="form-group form-group-sm">
											<select id="id_mobil" name="id_mobil" class="form-control select2" style="width: 70%;">
												<option></option>
												<?php
												...
												if (count($resnya01) > 0) {
													foreach ($resnya01 as $idx01 => $data01) {
														...
														echo '<option value="' . $data01['id_mobil'] . '" data-cabang="' . $data01['id_cabang'] . '" data-mobilnya="' . $gambarnya . '" data-nopol="' . $data01['plat_mobil'] . '">' . $data01['nama_mobil'] . '</option>';
													}
												}
												?>
											</select>
										...
										<div class="form-group">
											...
											<button type="button" class="btn btn-sm btn-success hide" id="modalMaps" data-toggle="modal" data-target="#fullscreenModal"><i class="fas fa-map"></i> Lihat Lokasi Mobil</button>
										</div>
									</div>
									...
									<div class="col-md-6">
										<div class="form-group form-group-sm">
											<select id="id_mobil" name="id_mobil" class="form-control select2" style="width: 70%;">
												<option></option>
												<?php
												$sqlnya01 = "SELECT a.id_mobil, a.plat_mobil, concat(a.nama_mobil, ' ', a.plat_mobil) as nama_mobil, a.id_cabang, a.attach_foto, b.inisial_cabang from pro_master_mobil a JOIN pro_master_cabang b ON a.id_cabang=b.id_master where a.is_active = 1";
												if ($seswil == '1' || $seswil == '2') {
													$sqlnya01 .= " and (a.id_cabang = '1' or a.id_cabang = '2')";
												} else {
													$sqlnya01 .= " and a.id_cabang = '" . $seswil . "'";
												}
												$resnya01 = $con->getResult($sqlnya01);
												$pathFile = $public_base_directory . '/files/uploaded_user/lampiran';
												if (count($resnya01) > 0) {
													foreach ($resnya01 as $idx01 => $data01) {
														...
														echo '<option value="' . $data01['id_mobil'] . '" data-cabang="' . $data01['id_cabang'] . '" data-mobilnya="' . $gambarnya . '" data-nopol="' . $data01['plat_mobil'] . '" data-inisial-cabang="' . $data01['inisial_cabang'] . '">' . $data01['nama_mobil'] . '</option>';
													}
												}
												?>
											</select>
					...
					<div class="table-responsive">
						<table class="table no-border table-info-event no-margin">
							<tbody>
								...
								<tr id="wrap-event-lastkm">
									<td>Kilometer Terakhir</td>
									<td class="text-center">:</td>
									<td id="info-event-lastkm"></td>
								</tr>
								<tr id="wrap-event-bensin">
									<td>Bensin Terakhir</td>
									<td class="text-center">:</td>
									<td id="info-event-bensin"></td>
								</tr>
								<tr id="wrap-event-meterbar">
									<td>&nbsp;</td>
									<td class="text-center">&nbsp;</td>
									<td id="info-event-meterbar"></td>
								</tr>
							</tbody>
						</table>
					</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="gform" name="gform" method="post" role="form" class="form-horizontal">
						<div class="row">
							<div class="col-md-10">
								<div class="form-group form-group-sm">
									...
									<div class="col-md-9">
										<input type="text" id="nama_mobil" name="nama_mobil" class="form-control" readonly />
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row colekstra01">
							<div class="col-md-10">
								<div class="form-group form-group-sm">
									<label class="control-label col-md-3">Kilometer Terakhir</label>
									<div class="col-md-4">
										<div class="input-group">
											<input type="text" id="last_km" name="last_km" class="form-control text-right" />
						...
						<div class="row colekstra01">
							<div class="col-md-10">
								<div class="form-group form-group-sm">
									<label class="control-label col-md-3">Sisa Bensin</label>
									<div class="col-md-4">
										<div class="input-group">
											<input type="text" id="bensin" name="bensin" class="form-control text-right" />
						...
						<div class="row colekstra01">
							<div class="col-md-10">
								<div class="form-group form-group-sm">
									...
									<div class="col-md-4">
										<meter id="meterbar" value="0" min="0" low="20" optimum="50" high="80" max="100"></meter>
									</div>
								</div>
							</div>
						</div>
	...
	<script>
		$(document).ready(function() {
			<?php if ($sesrole == '1' || $sesrole == '14') { ?>
				$("select#id_cabang").on("change", function() {
					$("#loading_modal").modal({
						keyboard: false,
						backdrop: 'static'
					});
					$("#id_mobil").val("").trigger("change");
					$("#modalMaps").addClass("hide");
					getMasterDataRuangan($("select#id_cabang").val());
				});

				function getMasterDataRuangan(id_cabang) {
					$.ajax({
						type: "POST",
						url: "./__get_master_data_mobil.php",
						data: {
							"q1": id_cabang
						},
						cache: false,
			...
			$("#last_km").number(true, 2, ".", ",");
			$("#bensin").number(true, 0, ".", ",");
			$("select#id_mobil").on("change", function() {
				let nilai = $("#id_mobil option:selected").data("mobilnya");
				if (nilai) {
					let pathFile = base_url + '/files/uploaded_user/lampiran' + nilai;
					let dataIcons =
						'<a href="' + pathFile + '" target="_blank" title="Lihat Gambar">' +
						'<img src="' + pathFile + '" />' +
						'</a>';
					$(".gambarnya-foto").html(dataIcons);
				} else {
					$(".gambarnya-foto").html("");
				}
			...
			$("#bensin").on("keyup input change", function() {
				let nilai = $(this).val();
				if (nilai >= 100) {
					nilai = 100;
					$("#bensin").val(100);
			...
			$("#modalMaps").on("click", function() {
				var mobil = document.getElementById("id_mobil");
				var selectedOption = mobil.options[mobil.selectedIndex];
				var cabang = document.getElementById("id_cabang");
				var selectedOption2 = cabang.options[cabang.selectedIndex];
				var inisial = selectedOption2.getAttribute("data-inisial-cabang");

				var nopol = selectedOption.getAttribute("data-nopol");
				nopol = nopol.replace(/\s*\(.*?\)\s*/g, '').trim();

				var gabungan = nopol + " " + inisial;

				var encodedNopol = encodeURIComponent(gabungan);
				var url = "https://app1.muliatrack.com/proenergi/track.php?token=0bda6306eb66d08406ab494f28f03c3f&nopol=" + encodedNopol;
				console.log(url)
			...
			$("#btnSbmt01").on("click", function() {
				...
				if ($("#id_mobil").val()) {
					...
					if (sesrole == "1" || sesrole == "14") {
						$("#modalMaps").removeClass("hide");
					}
					...
					$("#calendar").fullCalendar({
						...
						eventClick: function(calEvent, jsEvent, view) {
							if (view.name == "month") {
								...
								$("#eventModal").find("#info-event-bensin").html(calEvent.bensin);
								$("#eventModal").find("#info-event-meterbar").html(calEvent.meterbar);
								$("#eventModal").find("#info-event-lastkm").html(calEvent.last_km);
						...
						dayRender: function(date, cell, view) {
							...
							cellSbl.on("click", function() {
								...
								if (view.name == "month" && clkDate.getMonth() === defDate.getMonth()) {
									...
									$("form#gform").find("#idr").val($("#id_mobil").val());
									$("form#gform").find("#idc").val($("#id_mobil option:selected").data('cabang'));
									$("form#gform").find("#nama_mobil").val($("#id_mobil option:selected").text());
			...
			$("#btnSbmt02").on("click", function() {
				...
				if (isValid) {
					...
					$.post($base_url + "/web/action/peminjaman-mobil.php", $("form#gform").serializeArray(), function(data) {
						if (data.error) {
			...
			$("#eventModal").on("click", "#hapus-event", function() {
				swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.post($base_url + "/web/action/peminjaman-mobil.php", {
							act: 'delete',
							idk: eventId
						}, function(data) {
							if (data.error) {
				...
				$("#eventModal").one('hidden.bs.modal', function(e) {
					...
					$("#agenda_form_modal").one('shown.bs.modal', function(e) {
						$("#agenda_form_modal > .modal-dialog").addClass("loading");
						$.post($base_url + "/web/__get_mobil_event_data.php", {
							prm: eventId
						}, function(data) {
							$("form#gform").find(".form-group").removeClass("has-error");
							$("form#gform").find("label.error").remove();

							$("form#gform").find("#nama_mobil").val(data.nama_mobil);
