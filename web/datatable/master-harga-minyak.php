...
$sql .= " from pro_master_harga_minyak a join pro_master_area b on a.id_area = b.id_master join pro_master_produk c on a.produk = c.id_master) a where 1=1";
if ($q1 != "")
	$sql .= " and a.periode_awal = '" . tgl_db($q1) . "'";
if ($q2 != "")
	$sql .= " and a.id_area = '" . $q2 . "'";
if ($q3 != "" && $q3 != 2)
	$sql .= " and a.is_approved = '" . $q3 . "'";
if ($q4 != "")
	$sql .= " and a.produk = '" . $q4 . "'";
...
if ($tot_record <= 0) {
	...
	foreach ($result as $data) {
		...
		if ($data['is_approved'] == 1) $status = "Disetujui";
		else if ($data['is_evaluated'] == 1) $status = "Diverifikasi CEO";
