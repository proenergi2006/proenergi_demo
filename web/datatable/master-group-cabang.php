<?php
	...
	$p = new paging;
	$sql = "select * from pro_master_group_cabang where 1=1";
	
	if($q1 != "")
		$sql .= " and upper(group_wilayah) like '%".strtoupper($q1)."%'";
	if($q2 != "" && $q2 != 2)
	...
	$sql .= " order by is_active, id_gu limit ".$position.", ".$length;
	...
	if($tot_record <= 0){
		...
		foreach($result as $data){
			$count++;
			$linkEdit	= BASE_URL_CLIENT.'/add-master-group-cabang.php?'.paramEncrypt('idr='.$data['id_gu']);
			$linkHapus	= paramEncrypt("master_group_cabang#|#".$data['id_gu']);
