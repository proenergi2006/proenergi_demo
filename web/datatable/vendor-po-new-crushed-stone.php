...
if ($tot_record <= 0) {
    ...
    foreach ($result as $data) {
        ...
        $linkEdit    = BASE_URL_CLIENT . '/vendor-po-new-crushed-stone-add.php?' . paramEncrypt('idr=' . $data['id_master']);
        $linkDetail    = BASE_URL_CLIENT . '/vendor-po-new-crushed-stone-detail.php?' . paramEncrypt('idr=' . $data['id_master']);
        $linkCancel    = BASE_URL_CLIENT . '/vendor-po-crushed-stone-cancel.php?' . paramEncrypt('idr=' . $data['id_master']);
        $linkClose    = BASE_URL_CLIENT . '/vendor-po-crushed-stone-close.php?' . paramEncrypt('idr=' . $data['id_master']);
        ...
        $linkCetakSyop = BASE_URL_CLIENT . '/po-crushed-stone-new-terima-syop-view.php?' . paramEncrypt('idr=' . $data['id_master']);

        $linkTerima    = '
			<a class="margin-sm btn btn-action btn-primary" title="' . $titlenya . '" href="' . BASE_URL_CLIENT . '/vendor-po-crushed-stone-new-terima.php?' . paramEncrypt('idr=' . $data['id_master']) . '">
    ...
    <tr >
        ...
        <td class="text-right"> 
<p style="margin-bottom:0px;">PO : ' . number_format($data['harga_tebus']) . '</p></td>
        ...
        if ($showAction) {
            ...
            //$content .= '<a class="margin-sm btn btn-action btn-info" title="Edit" href="' . $linkEdit . '"><i class="fa fa-table"></i></a>';
            $content .= '<a class="margin-sm btn btn-action btn-info" title="Detail" href="' . $linkDetail . '"><i class="fa fa-table"></i></a>';
            ...
            if ($data['ceo_result'] == 1 && $data['revert_ceo'] == 0) {
				...
				<ul class="dropdown-menu" role="menu">
					...
					<li><a target="_blank" href="' . $linkCetakSyop . '"">Cetak</a></li>
