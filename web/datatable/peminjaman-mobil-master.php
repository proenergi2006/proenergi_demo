...
if ($q1 != "")
	$sql .= " and (upper(a.nama_mobil) like '%" . strtoupper($q1) . "%' or upper(a.plat_mobil) like '%" . strtoupper($q1) . "%' or upper(b.nama_cabang) like '%" . strtoupper($q1) . "%')";
...
if ($tot_record ==  0) {
	...
	foreach ($result as $data) {
		$count++;
		$linkView 	= BASE_URL_CLIENT . '/peminjaman-mobil-master-add.php?' . paramEncrypt('idr=' . $data['id_mobil']);
		$linkHapus	= paramEncrypt("master_mobil#|#" . $data['id_mobil']);
		...
		$sql2 = "SELECT * FROM pro_peminjaman_mobil WHERE id_mobil = '" . $data['id_mobil'] . "'";
		...
		$content .= '
				<tr class="clickable-row" data-href="' . $linkView . '">
					...
					<td class="text-center">' . $data['plat_mobil'] . '</td>
