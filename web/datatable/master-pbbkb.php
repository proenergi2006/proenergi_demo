<?php
	...
	$p = new paging;
	$sql = "select * from pro_master_pbbkb where 1=1";
	
	if($q1 != "")
		$sql .= " and nilai_pbbkb = '".$q1."'";
	if($q2 != "" && $q2 != 2)
	...
	$sql .= " order by is_active, nilai_pbbkb limit ".$position.", ".$length;
	...
	if($tot_record <= 0){
		...
		foreach($result as $data){
			$count++;
			$linkEdit	= BASE_URL_CLIENT.'/add-master-pbbkb.php?'.paramEncrypt('idr='.$data['id_master']);
			$linkHapus	= paramEncrypt("master_pbbkb#|#".$data['id_master']);
			$active		= ($data["is_active"] == 1)?"Active":"Not Active";
			$nilai		= $data['nilai_pbbkb'];
        	$content .= '
				<tr class="clickable-row" data-href="'.$linkEdit.'">
					<td class="text-center">'.$count.'</td>
					<td>'.$nilai.' %</td>
					<td>'.$data['ket_pbbkb'].'</td>
					<td>'.$active.'</td>
