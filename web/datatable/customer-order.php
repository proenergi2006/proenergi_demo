<?php
	...
	$p = new paging;
	$sql = "select a.*, b.nama_customer, b.kode_pelanggan from pro_permintaan_order a join pro_customer b on a.id_customer = b.id_customer 
			where 1=1";
	if ($sesrol == 18) {
		if (paramDecrypt($_SESSION['sinori'.SESSIONID]['id_wilayah']) or paramDecrypt($_SESSION['sinori'.SESSIONID]['id_group'])) {
	        ...
	        if (paramDecrypt($_SESSION['sinori'.SESSIONID]['id_wilayah']) and paramDecrypt($_SESSION['sinori'.SESSIONID]['id_group']))
	            $sqlCustomerWilayah = "select id_user from acl_user where (id_wilayah = '".paramDecrypt($_SESSION["sinori".SESSIONID]["id_wilayah"])."' or a.pic_user = '".paramDecrypt($_SESSION["sinori".SESSIONID]["id_user"])."')";
	        else if (!paramDecrypt($_SESSION['sinori'.SESSIONID]['id_wilayah']) and paramDecrypt($_SESSION['sinori'.SESSIONID]['id_group']))
	            $sqlCustomerWilayah = "select id_user from acl_user where (id_group = '".paramDecrypt($_SESSION["sinori".SESSIONID]["id_group"])."' or a.pic_user = '".paramDecrypt($_SESSION["sinori".SESSIONID]["id_user"])."')";
	...
	if($q1 != "")
		$sql .= " and (a.nomor_order = '".$q1."' or a.tanggal_order = '".tgl_db($q1)."' or b.kode_pelanggan = '".$q1."' or upper(b.nama_customer) like '%".strtoupper($q1)."%')";

	$tot_record = $con->num_rows($sql);
	$tot_page 	= ceil($tot_record/$length);
	$page		= ($start > $tot_page)?$start-1:$start; 
	$position 	= $p->findPosition($length, $tot_record, $page);
	$sql .= " order by a.is_delivered, a.id_pmnt_order desc limit ".$position.", ".$length;
	...
	if($tot_record <= 0){
		...
		foreach($result as $data){
			...
			$count++;
			$linkDetail	= BASE_URL_CLIENT.'/customer-order-detail.php?'.paramEncrypt('idr='.$data['id_pmnt_order']);
