...
$p = new paging;
$sql = "SELECT a.*, CONCAT(b.kode_pelanggan,' ',b.nama_customer) as nama_customer, c.fullname as marketingnya FROM pro_master_penerima_refund a JOIN pro_customer b ON a.id_customer=b.id_customer JOIN acl_user c ON b.id_marketing=c.id_user";

if ($role == '11') {
	$sql .= " AND b.id_marketing = '" . $id_user . "'";
} elseif ($role == '18' || $role == '7') {
	$sql .= " AND b.id_wilayah = '" . $wilayah . "'";
}
...
$sql .= " order by
a.is_bm ASC limit " . $position . ", " . $length;
...
if ($tot_record == 0) {
	...
	foreach ($result as $data) {
		...
		$dataParam = $data['nama_customer'] . '|' . $data['nama'] . '|' . $data['divisi'] . '|' . $data['bank'] . ' - ' . $data['kode_bank'] . '|' . $data['no_rekening'] . '|' . $data['atas_nama'] . '|' . $data['is_bm'] . '|' . $data['bm_by'] . '|' . tgl_indo($data['bm_date']) . " " . date("H:i:s", strtotime($data['bm_date'])) . '|' . $data['catatan_bm'] . '|' . $data['catatan_ceo'] . '|' . $data['is_ceo'] . '|' . $data['ceo_by'] . '|' . tgl_indo($data['ceo_date']) . " " . date("H:i:s", strtotime($data['ceo_date'])) . '|' . $data['no_ktp'] . '|' . $url_action_ktp . '|' . $url_action_npwp;
