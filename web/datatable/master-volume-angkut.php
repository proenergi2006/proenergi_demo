<?php
	...
	$p = new paging;
	$sql = "select * from pro_master_volume_angkut where 1=1";
	
	if($q1 != "")
		$sql .= " and volume_angkut = '".$q1."'";
	if($q2 != "" && $q2 != 2)
	...
	$sql .= " order by is_active, volume_angkut limit ".$position.", ".$length;
	...
	if($tot_record <= 0){
		...
		foreach($result as $data){
			$count++;
			$linkEdit	= BASE_URL_CLIENT.'/add-master-volume-angkut.php?'.paramEncrypt('idr='.$data['id_master']);
			$linkHapus	= paramEncrypt("master_volume_angkut#|#".$data['id_master']);
			$active		= ($data["is_active"] == 1)?"Active":"Not Active";
			$nilai		= number_format($data['volume_angkut']);
        	$content .= '
