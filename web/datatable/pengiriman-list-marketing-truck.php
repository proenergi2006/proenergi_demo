...
$sql = "select a.*, c.pr_pelanggan, i.nama_customer, e.alamat_survey, f.nama_prov, g.nama_kab, j.fullname, n.nama_transportir, n.nama_suplier, b.no_spj, k.nomor_plat, k.link_gps,
			...
			c.produk, b.tgl_kirim_po, h.nomor_poc, h.tanggal_poc, b.tgl_eta_po, b.jam_eta_po, b.mobil_po, c.no_do_acurate, c.nomor_lo_pr, m.id_wilayah as id_wilayah_po
...
/*$sql = "select a.*, c.pr_pelanggan, i.nama_customer, i.kode_pelanggan, e.alamat_survey, f.nama_prov, g.nama_kab, j.fullname, h.nomor_poc, 
			...
			where i.id_marketing = '".paramDecrypt($_SESSION["sinori".SESSIONID]["id_user"])."' and a.is_cancel = 0 and a.is_loaded = 1";*/


if ($sesrol == 18 || $sesrol == 7) {
	...
	$sql .= " and i.id_customer = '" . paramDecrypt($_SESSION["sinori" . SESSIONID]["id_user"]) . "'";
} else if ($sesrol == 24) {
	...
	foreach ($result as $data) {
		...
		if ($data['is_delivered']) {
			...
			if ($data['link_gps'] == 'OSLOG') {
				if ($data['id_wilayah_po'] == '2' && $data['tanggal_loading'] >= '2024-01-01') {
					$mobilnya = '<a style="color:black;" class="" data-mobil="' . $data['nomor_plat'] . '">' . $data['nomor_plat'] . '</a>';
			...
			if ($data['link_gps'] == 'OSLOG') {
				if ($data['id_wilayah_po'] == '2' && $data['tanggal_loading'] >= '2024-01-01') {
					$mobilnya = '<a class="openModalTracking" data-mobil="' . $data['nomor_plat'] . '">' . $data['nomor_plat'] . '</a>';
					$linkList 	= $data['id_dsd'];
					$classLink  = "openMonitoringDispatch";
					$titleLink  = "Monitoring Dispatch";
					$btnHistoryTrack = '';
					...
					$mobilnya = '<a class="getlokasimobil" data-mobil="' . $data['nomor_plat'] . '">' . $data['nomor_plat'] . '</a>';
					$linkList 	= paramEncrypt($data['id_dsd'] . "|#|1|#|" . $data['nomor_poc'] . "[]" . $data['nama_customer'] . "|#|" . $data['mobil_po'] . "|#|" . $data['volume_po']);
					$classLink  = "listStsT";
					$titleLink  = "Detail Pengiriman";
					$btnHistoryTrack = "";
		...
		$content .= '
				<tr>
					...
					<td class="text-left">
						<p style="margin-bottom:0px"><b>Delivery Order : </b>' . $delivery_order . '</b></p>
