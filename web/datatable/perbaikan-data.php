<?php
	...
	$p = new paging;
	$sql = "select a.*, b.nama_cabang from pro_pr a join pro_master_cabang b on a.id_wilayah = b.id_master where a.disposisi_pr = 6";
	
	if($q1 != "")
		$sql .= " and (a.tanggal_pr = '".tgl_db($q1)."' or a.nomor_pr = '".$q1."')";
	if($q2 != "")
	...
	if($tot_record <= 0){
		...
		foreach($result as $data){
			$count++;
			$linkDetail	= BASE_URL_CLIENT.'/perbaikan-data-detail.php?'.paramEncrypt('idr='.$data['id_pr']);
			$status		= "";
