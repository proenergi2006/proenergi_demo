...
$sqlnya = "
		...
		select a.id_master, a.id_datanya, 'Data Awal' as jenis_penambahan, 
		a.id_produk, concat(c.jenis_produk, ' - ', c.merk_dagang) as ket_produk, 
		a.id_terminal, concat(b.nama_terminal, ' ', b.tanki_terminal) as ket_terminal, 
		a.tanggal_inven, sum(in_inven) as nilai_jenis, a.keterangan, a.lastupdate_time 
		from new_pro_inventory_depot a 
		join pro_master_terminal b on a.id_terminal = b.id_master 
		join pro_master_produk c on a.id_produk = c.id_master 
		where id_jenis = 1
		...
		select a.id_master, a.id_datanya, 'Sales' as jenis_penambahan, 
		a.id_produk, concat(c.jenis_produk, ' - ', c.merk_dagang) as ket_produk, 
		a.id_terminal, concat(b.nama_terminal, ' ', b.tanki_terminal) as ket_terminal, 
		a.tanggal_inven, out_inven as nilai_jenis, a.keterangan, a.lastupdate_time 
		from new_pro_inventory_depot a 
		join pro_master_terminal b on a.id_terminal = b.id_master 
		join pro_master_produk c on a.id_produk = c.id_master 
		where id_jenis = 2
...
if ($q1 != "")
	$sqlnya .= " WHERE a.nama_terminal LIKE '%" . $q1 . "%'";
