<?php
	...
	$p = new paging;
	$sql = "select a.*, b.nama_transportir, b.nama_suplier, b.lokasi_suplier from pro_master_transportir_sopir a join pro_master_transportir b on a.id_transportir = b.id_master ";
	...
	if($q1 != "")
		$sql .= " and upper(a.nama_sopir) like '%".strtoupper($q1)."%'";
	if($q2 != "" && $q2 != 2)
	...
	if($tot_record <= 0){
		...
		foreach($result as $data){
			$count++;
			$linkEdit	= BASE_URL_CLIENT.'/add-master-transportir-sopir.php?'.paramEncrypt('idr='.$data['id_master']);
			$linkDetail	= BASE_URL_CLIENT.'/detil-master-transportir-sopir.php?'.paramEncrypt('idr='.$data['id_master']);
			$linkHapus	= paramEncrypt("master_transportir_sopir#|#".$data['id_master']);
        	...
        	$content .= '
				<tr class="clickable-row" data-href="'.$linkDetail.'">
					...
					<td class="text-center action">
						<a class="margin-sm btn btn-action btn-info" title="Edit" href="'.$linkDetail.'"><i class="fa fa-info-circle"></i></a>
