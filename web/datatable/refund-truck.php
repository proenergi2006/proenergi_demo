...
where k.refund_tawar != 0";

if ($id_role == '25') {
	if ($cabang) {
		$sql .= " and i.id_wilayah = '" . $cabang . "'";
	} else {
		$sql .= "";
	}
} else {
...
// $sql .= "  order by
// CASE WHEN a.total_refund = 0 AND a.disposisi_refund = 1 THEN a.id_refund END DESC,
// CASE WHEN a.total_refund = 0 AND a.disposisi_refund = 2 THEN a.disposisi_refund AND a.id_refund END DESC,
// CASE WHEN a.total_refund != 0 AND a.disposisi_refund = 3 THEN a.tgl_bayar END DESC,
// a.id_refund desc limit " . $position . ", " . $length;
...
if ($tot_record <= 0) {
	...
	foreach ($result as $data) {
		...
		// $id_invoice = array();
		// $tgl_bayar = array();
		// $status_invoice_1 = "Belum Lunas";
		// $status_invoice_2 = "";
		// $status_invoice_3 = "";
		// foreach ($arrid_invoice as $key) {
		// 	// ------------Invoice 1--------------
		// 	$sql_1 = "SELECT * FROM pro_invoice_admin WHERE id_invoice = '" . $key['id_invoice_1'] . "'";
		// 	$row_1 = $con->getRecord($sql_1);

		// 	if ($row_1['total_invoice'] == $row_1['total_bayar']) {
		// 		$sql_bayar_1 = "SELECT MAX(tgl_bayar) as tanggal_bayar FROM pro_invoice_admin_detail_bayar WHERE id_invoice='" . $key['id_invoice_1'] . "'";
		// 		$row_bayar_1 = $con->getRecord($sql_bayar_1);
		// 		$status_invoice_1 = "Lunas";
		// 	}
		...
		// 	if ($key['id_invoice_2'] != null) {
		// 		$sql_2 = "SELECT * FROM pro_invoice_admin WHERE id_invoice = '" . $key['id_invoice_2'] . "'";
		// 		$row_2 = $con->getRecord($sql_2);

		// 		if ($row_2['total_invoice'] == $row_2['total_bayar']) {
		// 			$sql_bayar_2 = "SELECT MAX(tgl_bayar) as tanggal_bayar FROM pro_invoice_admin_detail_bayar WHERE id_invoice='" . $key['id_invoice_2'] . "'";
		// 			$row_bayar_2 = $con->getRecord($sql_bayar_2);
		// 			$status_invoice_2 = "Lunas";
		// 		}
		// 	} else {
		// 		$status_invoice_2 = "Kosong";
		// 	}

		// 	// ------------Invoice 3--------------

		// 	if ($key['id_invoice_3'] != null) {
		// 		$sql_3 = "SELECT * FROM pro_invoice_admin WHERE id_invoice = '" . $key['id_invoice_3'] . "'";
		// 		$row_3 = $con->getRecord($sql_3);

		// 		if ($row_3['total_invoice'] == $row_3['total_bayar']) {
		// 			$sql_bayar_3 = "SELECT MAX(tgl_bayar) as tanggal_bayar FROM pro_invoice_admin_detail_bayar WHERE id_invoice='" . $key['id_invoice_3'] . "'";
		// 			$row_bayar_3 = $con->getRecord($sql_bayar_3);
		// 			$status_invoice_3 = "Lunas";
		// 		}
		// 	} else {
		// 		$status_invoice_3 = "Kosong";
		// 	}
		// 	array_push($tgl_bayar, $row_bayar_1['tanggal_bayar'], $row_bayar_2['tanggal_bayar'], $row_bayar_3['tanggal_bayar']);

		// 	$id_invoice[] = $row_1['no_invoice']  . $row_2['no_invoice']  . $row_3['no_invoice'];
		// 	$tgl_invoice = $row_1['tgl_invoice'];

		// 	$nomor_invoice = implode(", ", $id_invoice);
		// }
