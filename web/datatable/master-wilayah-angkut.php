<?php
	...
	$p = new paging;
	$sql = "select a.*, b.nama_prov, c.nama_kab from pro_master_wilayah_angkut a join pro_master_provinsi b on a.id_prov = b.id_prov 
			join pro_master_kabupaten c on a.id_kab = c.id_kab where 1=1";
	...
	if($tot_record <= 0){
		...
		foreach($result as $data){
			$count++;
			$linkEdit	= BASE_URL_CLIENT.'/add-master-wilayah-angkut.php?'.paramEncrypt('idr='.$data['id_master']);
			$linkHapus	= paramEncrypt("master_wilayah_angkut#|#".$data['id_master']);
			$active		= ($data["is_active"] == 1)?"Active":"Not Active";
        	$content .= '
				<tr class="clickable-row" data-href="'.$linkEdit.'">
					<td class="text-center">'.$count.'</td>
					<td>'.strtoupper($data['nama_prov']).'</td>
					<td>'.str_replace(array("KABUPATEN ","KOTA "), array("",""), $data['nama_kab']).'</td>
					<td>'.$data['wilayah_angkut'].'</td>
					<td>'.$active.'</td>
