<?php
	...
	if($q1 != "") {
		$sql .= "
				...
				(
					...
					a.jenis_oil like '%".$q1."%' or
					a.spesifikasi like '%".$q1."%' or
					a.konsumsi_volume like '%".$q1."%' or
					a.konsumsi_unit like '%".$q1."%' or
	...
	if($tot_record ==  0){
		...
		foreach($result as $data){
			$count++;
			$linkView 	= BASE_URL_CLIENT.'/database-lubricant-oil-add.php?'.paramEncrypt('idr='.$data['id_database_lubricant_oil']);
			$linkDel	= paramEncrypt("database_lubricant_oil#|#".$data['id_database_lubricant_oil']);

        	$content .= '
				<tr class="clickable-row" data-href="'.$linkView.'">
					<td class="text-center">'.$count.'</td>
					<td>'.$data['nama_customer'].'</td>
					<td>'.$data['jenis_oil'].'</td>
					<td>'.$data['spesifikasi'].'</td>
					<td>'.number_format($data['konsumsi_volume']).'</td>
					<td>'.$data['konsumsi_unit'].'</td>
