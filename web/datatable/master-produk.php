...
$p = new paging;
$sql = "select * from pro_master_produk where 1=1";

if ($q1 != "")
	$sql .= " and (upper(jenis_produk) like '%" . strtoupper($q1) . "%' or upper(merk_dagang) like '%" . strtoupper($q1) . "%')";
if ($q2 != "" && $q2 != 2)
...
if ($tot_record <= 0) {
	...
	foreach ($result as $data) {
		$count++;
		$linkDetail	= BASE_URL_CLIENT . '/detil-master-produk.php?' . paramEncrypt('idr=' . $data['id_master']);
		$linkEdit	= BASE_URL_CLIENT . '/add-master-produk.php?' . paramEncrypt('idr=' . $data['id_master']);
		$linkHapus	= paramEncrypt("master_produk#|#" . $data['id_master']);
		if ($data["is_active"] == 1) {
			$active = "Active";
		} else {
			$active = "Not Active";
