<?php
	...
	$p = new paging;
	$ar9 = array("9"=>"a.flag_approval, a.logistik_result", "7"=>"a.flag_approval, a.sm_result");
	$sql = "select a.*, b.nama_prov, c.nama_kab, d.nama_customer, d.kode_pelanggan, e.fullname, f.nama_cabang 
			...
			where d.id_wilayah = '".paramDecrypt($_SESSION['sinori'.SESSIONID]['id_wilayah'])."'";

	if(paramDecrypt($_SESSION['sinori'.SESSIONID]['id_role']) == 7)
		$sql .= " and a.flag_disposisi > 1";
	...
	$sql .= " order by ".$ar9[paramDecrypt($_SESSION['sinori'.SESSIONID]['id_role'])].", a.id_lcr desc limit ".$position.", ".$length;
	...
	if($tot_record ==  0){
		...
		foreach($result as $data){
			$count++;
			$alamatLoka = $data['alamat_survey']." ".str_replace(array("KABUPATEN ","KOTA "), array("",""), $data['nama_kab'])." ".$data['nama_prov'];
			$surveyor	= "";
			$arSurveyor	= json_decode($data['nama_surveyor'], true);
			$linkDetail	= BASE_URL_CLIENT.'/verifikasi-lcr-detail.php?'.paramEncrypt('idr='.$data['id_customer'].'&idk='.$data['id_lcr']);
			$background	= "";
        	...
        	$content .= '
				<tr class="clickable-row" data-href="'.$linkDetail.'"'.$background.'>
					...
					<td>
						...
						<p style="margin-bottom:0px">Truck Max : '.($data['max_truk']?$data['max_truk'].' KL':'-').'</p>
					</td>
					<td>'.$status.'</td>
					<td class="text-center">'.$attachment.'</td>
					<td class="text-center action"><a class="margin-sm btn btn-action btn-info" href="'.$linkDetail.'" title="Detail"><i class="fa fa-info-circle"></i></a></td>
				</tr>';
		} 
	} 
