									...
									<div class="form-group form-group-sm">
										...
										<div class="col-md-6">
											...
											if (isset($rsm['harga_poc'])) {
												...
												echo '<input type="hidden" id="harga_liter2" name="harga_liter2" class="form-control text-right" required ' . $readonly . ' readonly/>';
											}
											?>
										</div>
									</div>
				...
				if ($(this).val() != "" && $(this).val() != null) {
					...
					$.ajax({
						...
						success: function(data) {
							...
							$("#harga_liter2").val(data.harga_koma);
							if (data.refund != 0) {
								...
								$.ajax({
									...
									success: function(data) {
										if (data.error) {
											...
											// form.submit();
											var unblock = false;
											const remaining = Number(reminding);
											const total = Number(total_order);
											const ovup07 = Number(ov_up_07);
											const ovunder30 = Number(ov_under_30);
											const ovunder60 = Number(ov_under_60);
											const ovunder90 = Number(ov_under_90);
											const ovup90 = Number(ov_up_90);

											if (total > remaining) unblock = true;

											if (ovup07 > 0 || ovunder30 > 0 || ovunder60 > 0 || ovunder90 > 0 || ovup90 > 0) unblock = true;
