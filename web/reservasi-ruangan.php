...
<!DOCTYPE html>
<html lang="en">
<?php load_headHtml(BASE_PATH_CSS, BASE_PATH_JS, array("js" => array("jqueryUI", "formhelper"), "css" => array("jqueryUI", "formhelper"))); ?>
									...
									<div class="col-md-6">
										<div class="form-group form-group-sm">
											<select id="id_cabang" name="id_cabang" class="form-control select2" style="width:100%;">
												<option></option>
												<?php $con->fill_select("id_master", "nama_cabang", "pro_master_cabang", $seswil, "where is_active=1", "id_master", false); ?>
											</select>
										</div>
										<div class="form-group form-group-sm">
											<select id="id_ruangan" name="id_ruangan" class="form-control select2" style="width:100%;">
												<option></option>
												<?php
												$sqlnya01 = "select id_ruangan, nama_ruangan, attach_foto from pro_master_ruangan where is_active = 1";
												...
												if (count($resnya01) > 0) {
													foreach ($resnya01 as $idx01 => $data01) {
														...
														echo '<option value="' . $data01['id_ruangan'] . '" data-gambar="' . $gambarnya . '">' . $data01['nama_ruangan'] . '</option>';
													}
												}
												?>
											</select>
					...
					<div class="table-responsive">
						<table class="table no-border table-info-event no-margin">
							<tbody>
								...
								<tr id="wrap-event-personel">
									<td>Personel</td>
									<td class="text-center">:</td>
									<td id="info-event-personel"></td>
								</tr>
				...
				<div class="modal-body">
					<form id="gform" name="gform" method="post" role="form" class="form-horizontal">
						<div class="row">
							<div class="col-md-10">
								<div class="form-group form-group-sm">
									...
									<div class="col-md-9">
										<input type="text" id="nama_ruangan" name="nama_ruangan" class="form-control" readonly />
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-md-10">
								<div class="form-group form-group-sm">
									<label class="control-label col-md-3">Personel *</label>
									<div class="col-md-9">
										<textarea id="personel" name="personel" class="form-control" required style="height:90px;"></textarea>
									</div>
								</div>
							</div>
						</div>
		...
		$(document).ready(function() {
			<?php if ($sesrole == '1' || $sesrole == '14') { ?>
				...
				function getMasterDataRuangan(id_cabang) {
					$.ajax({
						type: "POST",
						url: "./__get_master_data_ruangan.php",
						data: {
							"q1": id_cabang
						},
						cache: false,
			...
			$("select#id_ruangan").on("change", function() {
				let nilai = $("#id_ruangan option:selected").data("gambar");
				if (nilai) {
					$(".gambarnya").removeClass("hide");
					let pathFile = base_url + '/files/uploaded_user/lampiran' + nilai;
					let dataIcons =
						'<div style="margin-bottom:15px;">' +
						'<a href="' + pathFile + '" target="_blank" title="download file"><img src="' + pathFile + '" style="width:250px;" /></a>' +
						'</div>';
					$(".gambarnya-foto").html(dataIcons);
				} else {
					$(".gambarnya").addClass("hide");
					$(".gambarnya-foto").html("");
				}
					...
					$("#calendar").fullCalendar({
						...
						eventClick: function(calEvent, jsEvent, view) {
							if (view.name == "month") {
								...
								$("#eventModal").find("#info-event-personel").html(calEvent.personel.replace(/\n/g, "<br />"));
						...
						dayRender: function(date, cell, view) {
							...
							cellSbl.on("click", function() {
								...
								if (view.name == "month" && clkDate.getMonth() === defDate.getMonth()) {
									...
									$("form#gform").find("#idr").val($("#id_ruangan").val());
									$("form#gform").find("#nama_ruangan").val($("#id_ruangan option:selected").text());
			...
			$("#btnSbmt02").on("click", function() {
				...
				if (isValid) {
					...
					$.post($base_url + "/web/action/reservasi-ruangan.php", $("form#gform").serializeArray(), function(data) {
						if (data.error) {
			...
			$("#eventModal").on("click", "#hapus-event", function() {
				swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.post($base_url + "/web/action/reservasi-ruangan.php", {
							act: 'delete',
							idk: eventId
						}, function(data) {
							if (data.error) {
				...
				$("#eventModal").one('hidden.bs.modal', function(e) {
					...
					$("#agenda_form_modal").one('shown.bs.modal', function(e) {
						$("#agenda_form_modal > .modal-dialog").addClass("loading");
						$.post($base_url + "/web/__get_event_data.php", {
							prm: eventId
						}, function(data) {
							$("form#gform").find(".form-group").removeClass("has-error");
							$("form#gform").find("label.error").remove();
							...
							$("form#gform").find("#idr").val(data.id_ruangan);
