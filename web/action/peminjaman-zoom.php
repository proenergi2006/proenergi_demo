...
if ($act == "add") {
	...
	if ($id_zoom == "" || $tgl_reservasi == "" || $jam_mulai == "" || $jam_selesai == "" || $departmen == "" || $keperluan == "") {
		$con->close();
		...
		$sql1 = "
				insert into pro_peminjaman_zoom(id_zoom, id_user, id_cabang, tanggal_peminjaman, start_jam_peminjaman, end_jam_peminjaman, departmen, keperluan, created_time, created_by, deleted_time) values 
				('" . $id_zoom . "', '" . $id_user . "', '" . $id_cabang . "', '" . $tgl_reservasi . "', '" . $jam_mulai . ":00', '" . $jam_selesai . ":00',  '" . $departmen . "', '" . $keperluan . "', NOW(), '" . paramDecrypt($_SESSION['sinori' . SESSIONID]['fullname']) . "', NULL)
			";
	...
	if ($id_zoom == "" || $tgl_reservasi == "" || $jam_mulai == "" || $jam_selesai == "" || $departmen == "" || $keperluan == "") {
		...
		$sql1 = "
				update pro_peminjaman_zoom set tanggal_peminjaman = '" . $tgl_reservasi . "', start_jam_peminjaman = '" . $jam_mulai . ":00', end_jam_peminjaman = '" . $jam_selesai . ":00', 
				departmen = '" . $departmen . "', keperluan = '" . $keperluan . "', created_time = NOW() where id_peminjaman = '" . $id_peminjaman . "'
			";
		$res1 = $con->setQuery($sql1);
		$oke  = $oke && !$con->hasError();
	...
	$sql1 = "delete from pro_peminjaman_zoom where id_peminjaman = '" . paramDecrypt($id_peminjaman) . "'";
	$res1 = $con->setQuery($sql1);
	$oke  = $oke && !$con->hasError();
