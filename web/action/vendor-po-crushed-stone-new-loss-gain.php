...
if ($act == 'update') {
    ...
    if ($id1nya) {
        ...
        $sql2 =
            "
                        insert into new_pro_inventory_gain_loss_crushed_stone (id_po_supplier, volume_po,  volume_terima, jenis, volume, file_upload, file_upload_ori, ket, disposisi_gain_loss, created_time, created_ip, 
                         created_by) (
                            select '" . $idr . "', '" . $volume_po_loss_gain  . "', '" . $volume_terima_loss_gain  . "', '" . $jenis  . "', '" . $volume_loss_gain  . "', '" . $fileName . "', '" . $fileOriginName . "', '" . $ket_loss_gain . "', 2,NOW(), '" . $_SERVER['REMOTE_ADDR'] . "', '" . paramDecrypt($_SESSION['sinori' . SESSIONID]['fullname']) . "' 
                        )";
        $con->setQuery($sql2);
