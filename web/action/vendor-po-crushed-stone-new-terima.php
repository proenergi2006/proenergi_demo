...
if ($act == 'cek') {
    ...
    $kuenya     = "select LPAD(cast((select nextval(new_pro_inventory_vendor_po_receive_crushed_stone1_seq)) as varchar(10)), 9, '0') as idnya";
    ...
    if ($idnya02) {
        if ($filePhoto1) {
            ...
            $fileName         = $folder . '/terimaposupplierbatu_' . $idnya02 . '_' . md5(basename($filePhoto1, $fileExt)) . '.' . $fileExt;
        ...
        $sql1 = "
				insert into new_pro_inventory_vendor_po_crushed_stone_receive(id_po_receive, id_po_supplier, nama_pic, tgl_terima, volume_bol, volume_terima, harga_tebus, file_upload, file_upload_ori, 
        ...
        $sql3 = "
				insert into new_pro_inventory_harga_tebus_crushed_stone (id_po_supplier, id_po_receive, id_produk, id_terminal, tgl_terima, harga_tebus) (
					select '" . $idnya01 . "', '" . $idnya02 . "', id_produk, id_terminal, '" . tgl_db($tgl_terima) . "', '" . $harga_tebus . "' 
					from new_pro_inventory_vendor_po_crushed_stone where id_master = '" . $idnya01 . "'
				)";
        $con->setQuery($sql3);
        $oke  = $oke && !$con->hasError();
        ...
        if ($oke) {
            ...
            header("location: " . BASE_URL_CLIENT . "/vendor-po-crushed-stone-new-terima.php?" . paramEncrypt("idr=" . $idnya01));
            exit();
        } else {
            $con->rollBack();
            $con->clearError();
    ...
    if ($filePhoto1) {
        $cek01 = "
				select a.file_upload, a.file_upload_ori from new_pro_inventory_vendor_po_crushed_stone_receive a 
    ...
    $cek02 = "select id_produk, id_terminal, harga_tebus from new_pro_inventory_vendor_po_crushed_stone a where a.id_master = '" . $idnya01 . "'";
    ...
    $sql1 = "update new_pro_inventory_vendor_po_crushed_stone_receive set nama_pic = '" . $nama_pic . "', tgl_terima = '" . tgl_db($tgl_terima) . "', 
    ...
    if ($res02['harga_tebus'] != $harga_tebus) {
        $sql3 = "
				insert into new_pro_inventory_harga_tebus_crushed_stone (id_po_supplier, id_po_receive, id_produk, id_terminal, tgl_terima, harga_tebus) values (
