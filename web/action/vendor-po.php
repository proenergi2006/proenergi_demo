<?php
	...
	//echo '<pre>'; print_r($dt7); exit;

	if($dt1 == "" || $dt2 == "" || $dt3 == "" || $dt4 == "" || $dt5 == "" || $dt6 == ""){
		$con->close();
		...
		$cek1 = "select * from pro_inventory_vendor_po where nomor_po = '".$dt2."'";
		$row1 = $con->getRecord($cek1);

		if($row1) {
			$msg = "GAGAL_UBAH";
			$idr = $row1['id_master'];
			$sql = "update pro_inventory_vendor_po set harga_tebus = '".$dt8."', in_inven_po = '".$dt10."', lastupdate_time = NOW(), 
			...
			$msg = "BERHASIL_MASUK";
			$sql = "insert into pro_inventory_vendor_po(tanggal_inven, nomor_po, id_produk, id_area, id_vendor, id_terminal, in_inven_po, is_diterima, harga_tebus, created_time, created_ip, 
					created_by) values ('".tgl_db($dt1)."', '".$dt2."', '".$dt3."', '".$dt4."', '".$dt5."', '".$dt6."', '".$dt10."',0, '".$dt8."', NOW(), 
