<?php
	...
	$idk	= htmlspecialchars(paramDecrypt($_POST["idk"]), ENT_QUOTES);
	...
	if($nama_customer == "" || $alamat_customer == "" || $prov_customer == "" || $kab_customer == "" || $telp_customer == "" || $email_customer == "" || $update_by == ""){
		...
		$sql1 = "
			update pro_customer set nama_customer = '".$nama_customer."', alamat_customer = '".$alamat_customer."', prov_customer = '".$prov_customer."', kab_customer = '".$kab_customer."', 
			telp_customer = '".$telp_customer."', fax_customer = '".$fax_customer."', email_customer = '".$email_customer."', website_customer = '".$website_customer."', tipe_bisnis = '".$tipe_bisnis."', 
			tipe_bisnis_lain = '".$tipe_bisnis_lain."', ownership = '".$ownership."', ownership_lain = '".$ownership_lain."', jenis_payment = '".$jenis_payment."', jenis_net = '".$jenis_net."', 
			top_payment = '".$top_payment."', nomor_sertifikat = '".$nomor_sertifikat."', nomor_npwp = '".$nomor_npwp."', nomor_siup = '".$nomor_siup."', nomor_tdp = '".$nomor_tdp."',  
			dokumen_lainnya = '".$dokumen_lainnya."', 
			lastupdate_time = NOW(), lastupdate_ip = '".$_SERVER['REMOTE_ADDR']."', lastupdate_by = '".$update_by."', 
			need_update = 0, postalcode_customer = '".$postalcode_customer."', induk_perusahaan = '".$induk_perusahaan."', credit_limit = '".$credit_limit."', 
			kecamatan_customer = '".$kecamatan_customer."', kelurahan_customer = '".$kelurahan_customer."' 
		...
		$sql5 = "update pro_customer_update set flag_edited = 1, tgl_edited = NOW() where id_cu = '".$idk."'";
		$con->setQuery($sql5);
		$oke  = $oke && !$con->hasError();
		$url  = BASE_URL_CLIENT."/verifikasi-permohonan.php";
