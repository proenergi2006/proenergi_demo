...
class XLSXWriter
{
	...
	public function setColumnIndex($column=0) { $this->column_index=$column; }
	...
	public function writeSheetHeaderExt($sheet_name, array $header_types){
		...
		$sheet = &$this->sheets[$sheet_name];
		foreach($header_types as $v){
			array_push($sheet->columns, $this->addCellFormat($v));
		}
		$header_row = array_keys($header_types);
		$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="12.1" outlineLevel="0" r="'.($sheet->row_count+1).'">');
		foreach ($header_row as $k => $v) {
			$this->writeCell($sheet->file_writer, $sheet->row_count, $k, $v, $cell_format_index = '0');
		}
		$sheet->file_writer->write('</row>');
		$sheet->row_count++;
		$this->current_sheet = $sheet_name;
	...
	public function writeSheetRow($sheet_name, array $row)
	{
		...
		$column_count = 0;
		foreach ($row as $k => $v) {
			$this->writeCell($sheet->file_writer, $sheet->row_count, $column_count, $v, $sheet->columns[$column_index]);
			$column_count++;
			$column_index++;
		}
		$sheet->file_writer->write('</row>');
