...
var DayGrid = Grid.extend({
	...
	renderDates: function(isRigid) {
		...
		var row;
		var i, cell;
		var type = this.view.type, is_myCalendar = this.view.options.myCustomCalendar;

		for (row = 0; row < rowCnt; row++) {
			var isLastRow = (type == "month" && is_myCalendar && row == (rowCnt - 1))?true:false;
			html += this.dayRowHtml(row, isRigid, isLastRow);
		}
		this.el.html(html);
	...
	dayRowHtml: function(row, isRigid, isLastRow) {
		var view = this.view;
	...
	computeColHeadFormat: function() {
		var is_myCalendar = this.view.options.myCustomCalendar;
		if (this.rowCnt > 1 && is_myCalendar) { // more than one week row. day numbers will be in each cell
			return 'dddd'; // "Sat"
		}
		else if (this.rowCnt > 1 && !is_myCalendar) { // more than one week row. day numbers will be in each cell
			return 'ddd'; // "Sat"
		}
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content fcc-content-normal">' +
					(this.isRTL ?
						titleHtml + ' ' + timeHtml : // put a natural space in between
						timeHtml + ' ' + titleHtml   //
						) +
				'</div>' +
				'<div class="fc-content fcc-content-responsive">'+timeHtml+'</div>' +
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		if(options.myCustomCalendar)
			content.addClass("fcc-container");
	...
	function updateHeaderTitle() {
		var is_myCalendar = currentView.options.myCustomCalendar;
		if(currentView.type == "month" && is_myCalendar){
			var title = currentView.title.split(" ");
			header.updateTitle(title[0]+"<div class=\"small-title\">"+title[1]+"</div>");
		} else
...
function Header(calendar, options) {
	...
	function render() {
		if(options.myCustomCalendar){
			var dom1 = $("<div>", {"class":"fcc-info"});
			var dom2 = $("<div>", {"class":"row"}).appendTo(dom1);
			var dom3 = $("<div>", {"class":"col-xs-2"}).appendTo(dom2).append(renderSection('left'));
			var dom4 = $("<div>", {"class":"col-xs-8"}).appendTo(dom2).append(renderSection('center'));
			var dom5 = $("<div>", {"class":"col-xs-2"}).appendTo(dom2).append(renderSection('right'));
			el = dom1;
		} else{
	...
	function renderSection(position) {
		if(options.myCustomCalendar){
			var dom1 = $();
			if(position == "left"){
				var dom1 = $("<div>", {"class":"text-left fcc-toolbar-left"})
				var dom2 = $("<i>", {"class":"fa fa-caret-left"}).appendTo(dom1);
				dom1.click(function(e){
					calendar["prev"]();
				});				
			} else if(position == "right"){
				var dom1 = $("<div>", {"class":"text-right fcc-toolbar-right"})
				var dom2 = $("<i>", {"class":"fa fa-caret-right"}).appendTo(dom1);
				dom1.click(function(e){
					calendar["next"]();
				});
			} else if(position == "center"){
				var dom1 = $("<h2>", {"class":"fcc-title text-center"})
			}
			return dom1;
		} else{
	...
	function updateTitle(text) {
		var is_myCalendar = options.myCustomCalendar;
		if(is_myCalendar)
...
var BasicView = View.extend({
	...
	renderHtml: function() {
		var is_myCalendar = this.options.myCustomCalendar;
		return '' +
			'<table '+(is_myCalendar?'class="table-main"':'')+'>' +
				'<thead class="fc-head">' +
					'<tr>' +
