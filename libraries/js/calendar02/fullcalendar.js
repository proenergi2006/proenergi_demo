...
var DayTableMixin = FC.DayTableMixin = {
	...
	computeColHeadFormat: function() {
		var is_myCalendar = this.view.opt('myCustomCalendar');
		// if more than one week row, or if there are a lot of columns with not much space,
		// put just the day numbers will be in each cell
		if ((this.rowCnt > 1 || this.colCnt > 10) && !is_myCalendar) {
			return 'ddd'; // "Sat"
		}
		else if ((this.rowCnt > 1 || this.colCnt > 10) && is_myCalendar) {
			return 'dddd'; // "Sat"
		}
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderDates: function(isRigid) {
		...
		var row;
		var col;
		var type = this.view.type, is_myCalendar = this.view.opt("myCustomCalendar");

		for (row = 0; row < rowCnt; row++) {
			var isLastRow = (type == "month" && is_myCalendar && row == (rowCnt - 1))?true:false;
			html += this.renderDayRowHtml(row, isRigid, isLastRow);
		}
		this.el.html(html);
	...
	renderDayRowHtml: function(row, isRigid, isLastRow) {
		var view = this.view;
...
function Toolbar(calendar, toolbarOptions) {
	...
	function render() {
		...
		if (sections) {
			if (!el) {
				if(calendar.opt('myCustomCalendar')){
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if(calendar.opt('myCustomCalendar')){
						if (buttonName == 'title') {
							groupChildren = groupChildren.add($('<h2 class="fcc-title text-center">&nbsp;</h2>')); // we always want it to take up height
							isOnlyButtons = false;
						}
						else {
							...
							if(buttonClick){
								if(buttonName == 'prev'){
									sectionEl.addClass('text-left fcc-toolbar-left');
									innerHtml = '<a class="btn btn-default"><i class="fa fa-caret-left"></i></a>';
								}
								else if(buttonName == 'next'){
									sectionEl.addClass('text-right fcc-toolbar-right');
									innerHtml = '<a class="btn btn-default"><i class="fa fa-caret-right"></i></a>';
								}
								button = $(innerHtml).click(function(ev){
...
Calendar.mixin({
	...
	initialRender: function() {
		...
		if(this.opt('myCustomCalendar'))
			this.contentEl.addClass("fcc-container");
...
var BasicView = FC.BasicView = View.extend({
	...
	renderSkeletonHtml: function() {
		var is_myCalendar = this.opt('myCustomCalendar');
		return '' +
